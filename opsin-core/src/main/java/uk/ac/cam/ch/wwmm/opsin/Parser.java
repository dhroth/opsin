...
class Parser {
     ...
     */
    Pair<List<Element>, List<Parse>> parse(NameToStructureConfig n2sConfig, String name) throws ParsingException {
        ...
        List<Parse> resultParses = new ArrayList<Parse>();
        ...
        return new Pair<List<Element>, List<Parse>>(results, resultParses);
    }
    ...
    void writeWordXML(Element wordEl, List<String> tokens, List<List<Character>> annotations) throws ParsingException {
        ...
        for (String token : tokens) {
            ...
            Character annotation = annotations.get(annotNumber).get(annotPos);
            //System.out.print(String.format("%s%02x|", token, (int) annotation));
            Element tokenElement = resourceManager.makeTokenElement(token, annotation);
